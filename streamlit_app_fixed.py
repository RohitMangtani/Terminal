    ...
    with open(".env", "r") as f:
        ...
        for line in env_contents.splitlines():
            if line.startswith("OPENAI_API_KEY="):
                ...
                st.sidebar.success(
                    f"âœ… OpenAI API key loaded: {OPENAI_API_KEY[:4]}...{OPENAI_API_KEY[-4:]}"
                )
                break
        else:
            OPENAI_API_KEY = None
            st.error("âŒ ERROR: OPENAI_API_KEY not found in .env file")
except Exception as e:
    st.error(f"âŒ ERROR reading .env file: {str(e)}")
    OPENAI_API_KEY = None
...
if not OPENAI_API_KEY:
    ...
    if OPENAI_API_KEY:
        st.sidebar.success(f"âœ… OpenAI API key loaded from environment")
    else:
        st.error("âŒ ERROR: OpenAI API key not found in environment")
        st.info(
...
def display_news_feed():
    ...
    with col2:
        ...
        if st.session_state.live_refresh:
            st.info("Auto-updating as new headlines appear âš¡")
        else:
...
def get_table_download_link(df, filename="data.csv", link_text="Download CSV"):
    ...
    href = (
        f'<a href="data:file/csv;base64,{b64}" download="{filename}">â¬‡ï¸ {link_text}</a>'
    )
    return href
